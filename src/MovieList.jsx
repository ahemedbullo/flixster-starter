...
function MovieList({ searchQuery, isNowPlaying, onMovieClick, sort }) {
  //passing in props
  ...
  const fetchMovies = async (reset = false) => {
    //just sets the default to reset
    ...
    if (!isNowPlaying && searchQuery !== "") {
      url = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&page=${pageNum}&query=${encodeURIComponent(
        searchQuery
      )}`; //api end point when searching
    }
    ...
    if (reset) {
      //this checks if I am now searching for a new movie if so just gives me the result or data of the new search
  ...
  useEffect(() => {
    //as the effect of a new search
    if (
      searchQuery !== prevSearchQuery ||
      isNowPlaying !== prevIsNowPlaying ||
      sort !== prevSort
    ) {
      //checking if the new searchQuery is the same as the previous one
    ...
    <div>
      <div className="movie-list">
        {movies &&
          movies.map(
            (
              movie //if there is data in movies it goes through the list of movies and creates a new card per movie
            ) => (
              <div key={`${movie.id}-${Math.random()}`}>
                ...
                <MovieCard /* for every movie it passes this data in */
                  ...
                  movie={
                    movie
                  } /* Passes each Movie and it's information as a prop */
                  onClick={
                    onMovieClick
                  } /* OnClick prop is passed so when the movie card is clicked */
                />
              </div>
            )
          )}
